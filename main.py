...
plt.scatter(XDisp, YDisp, c="red")
plt.scatter(XTrans, YTrans, c="green")
plt.scatter(XLoad, YLoad, c="blue")
plt.scatter(XFlood, YFlood, c="#e377c2")
...
plt.plot(getRpm(FrDispToTrans), getGasflow(FlDispToTrans, FrDispToTrans), color='r')
plt.plot(getRpm(FrTransToLoad), getGasflow(FlTransToLoad, FrTransToLoad), color='g')
plt.plot(getRpm(FrLoadToFlood), getGasflow(FlLoadToFlood, FrLoadToFlood), color='b')
...
for i in range(0, amountOfPicutures):
    ...
    experimentsFlood.append({'gas_flow': XLoad[i], 'rpm': YLoad[i]})
